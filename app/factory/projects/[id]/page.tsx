...
import AssemblyLinePanel from "./AssemblyLinePanel";
import ProductSynthesisPanel from "./ProductSynthesisPanel";
...
const tabs=["Overview","Assembly Line","Product Synthesis","Build Brief","Architecture","Economics","Features","Automations","Data","Integrations","Prototype","Activity"];
...
export default async function ProjectWorkspace({params}:{params:Promise<{id:string}>}){
  ...
  return <main>
    ...
    <section className="page-hero factory-hero"><p className="eyebrow">Factory Project / {label(project?.project_type)||"Project"}</p><h1>{name} <span>workspace.</span></h1><p>{project?.summary||"One operating record for discovery, product architecture, economics, automations, prototype work, approvals, and production history."}</p></section>
    <section className="section factory-workspace">
      <div className={styles.tabs}>{tabs.map((t,i)=><a href={t==="Assembly Line"?"#assembly-line":t==="Product Synthesis"?"#product-synthesis":t==="Architecture"?"#architecture":t==="Economics"?"#economics":"#overview"} key={t} className={i===0?styles.active:""}>{t}</a>)}</div>
      <div className={styles.summary} id="overview"><article><span className="product-tag">Stage</span><strong>{stage}</strong><p>{project?.progress??0}% through the current Factory lifecycle.</p></article><article><span className="product-tag">Infrastructure</span><strong>{infrastructure}</strong><p>Products remain in shared infrastructure until economics, security, scale or maturity justify graduation.</p></article><article><span className="product-tag">Next Gate</span><strong>Architecture + Economics</strong><p>{next}</p></article></div>
      <div className={styles.columns}><section className={styles.panel}><span className="product-tag">Product specification</span><h2>What the Factory produces</h2><div className={styles.checks}><div><b>01</b><span><strong>Build Brief</strong><small>Problem, users, outcome, requirements and constraints.</small></span></div><div><b>02</b><span><strong>Application Map</strong><small>Screens, roles, permissions, workflows and data.</small></span></div><div><b>03</b><span><strong>Automation Map</strong><small>Triggers, conditions, actions, approvals and exceptions.</small></span></div><div><b>04</b><span><strong>Economic Gate</strong><small>Who pays, pricing, cost, margin, break-even and graduation threshold.</small></span></div></div></section><section className={`${styles.panel} ${styles.automation}`}><span className="product-tag">Automation direction</span><h2>Work the system can perform</h2><p>{project?.automation_summary||"Automation candidates are generated from the discovery intake and refined during architecture review."}</p><div className={styles.automationRow}><strong>Factory rule</strong><small><b>Trigger:</b> Events, schedules, incoming data, or status changes</small><small><b>Action:</b> Evaluate → act → log → notify</small><small><b>Human control:</b> Required for consequential approvals and exceptions</small></div></section></div>
      <AssemblyLinePanel projectSlug={id}/>
      <ProductSynthesisPanel projectSlug={id}/>
      <ArchitecturePanel projectSlug={id}/>
      <EconomicsPanel projectSlug={id}/>
    </section>
  </main>
}
